...
class AncillaryInterpolationPU(EOProcessingUnit):

    def run(
        self,
        inputs: MappingDataType,
        anc_lat: np.ndarray = None,
        anc_lon: np.ndarray = None,
        anc_band_name: str = None,
        anc_attrs: dict = None,
        dtype: str = None,
        **kwargs,
    ) -> MappingDataType:
        ...
        result_data = da.map_blocks(
            ...
            dtype=anc_band.data.dtype,
            meta=np.array((), dtype=anc_band.data.dtype),
            **kwargs,
        )
        result = xr.DataTree()
