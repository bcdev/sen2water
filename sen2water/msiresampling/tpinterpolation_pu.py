...
class TpInterpolationPU(EOProcessingUnit):
    ...
    def run(
        ...
        band_name: str = None,
        band_idx: int = None,
        band_long_name: str = None,
        resolution: int = 60,
        tp_resolution: int = 5000,
        image_chunksize: Tuple[int, int] = None,
        **kwargs,
    ) -> MappingDataType:
        angle_data = inputs["l1c"][band_name][band_idx].values
        dummy_band = inputs["dummy_grid"]["data"]
        result_data = da.map_blocks(
            ...
            image_shape=dummy_band.data.shape,
            image_chunksize=image_chunksize,
            dtype=angle_data.dtype,
            meta=np.array((), dtype=angle_data.dtype),
            **kwargs,
        )
        result = xr.DataTree()
        result[band_name] = xr.DataArray(
            result_data,
            dims=dummy_band.dims,
            attrs=inputs["l1c"][band_name].attrs | { "long_name": band_long_name },
        )
