...
class MsiPixelClassification(BlockAlgorithm):
    ...
    def do_classification(
        ...
        result[self.is_invalid(self, toa)] = ic.FLAG_MASK_INVALID
        result[self.is_cloud_sure(self, toa)] = ic.FLAG_MASK_CLOUD_SURE
        result[self.is_cloud_ambiguous(self, toa)] = ic.FLAG_MASK_CLOUD_AMBIGUOUS
        result[self.is_cloud(self, toa)] = ic.FLAG_MASK_CLOUD
        result[self.is_cirrus(self, toa)] = ic.FLAG_MASK_CIRRUS_SURE
        result[self.is_cirrus_ambiguous(self, toa)] = ic.FLAG_MASK_CIRRUS_AMBIGUOUS
        result[self.is_bright_white(self, toa)] = ic.FLAG_MASK_BRIGHTWHITE
        result[self.is_clear_snow(self, toa)] = ic.FLAG_MASK_CLEAR_SNOW
        # result[self.is_clear_land(self, toa)] = ic.FLAG_MASK_CLEAR_LAND
        # result[self.is_clear_water(self, toa)] = ic.FLAG_MASK_CLEAR_WATER
    ...
    @staticmethod
    def is_cloud_sure(self, toa: tuple[DataTree, ...]) -> bool:
        tc4_cirrus_value = self.tc4_cirrus_value(self, toa)
        tc4_value = self.tc4_value(self, toa)
        ndwi_value = self.ndwi_value(self, toa)

        gcw = tc4_cirrus_value < ic.GCW_THRESH
        tcw = self._and(tc4_value < ic.TCW_TC_THRESH, ndwi_value < ic.TCW_NDWI_THRESH)
        is_b3_b11_water = self.is_b3_b11_water(self, toa)
        ...
        gcl = self._and(self._not(is_b3_b11_water),
                        ...
                        self.vis_bright_value(self, toa) > ic.VISBRIGHT_THRESH)

        is_invalid = self.is_invalid(self, toa)
        is_clear_snow = self.is_clear_snow(self, toa)

        return self._and(self._not(is_invalid), self._not(is_clear_snow), self._or(acw, gcl))

    @staticmethod
    def is_cloud_ambiguous(self, toa: tuple[DataTree, ...]) -> bool:
        tc4_cirrus_value = self.tc4_cirrus_value(self, toa)
        vis_bright_value = self.vis_bright_value(self, toa)
        is_b3_b11_water = self.is_b3_b11_water(self, toa)
        ...
        is_invalid = self.is_invalid(self, toa)
        is_clear_snow = self.is_clear_snow(self, toa)
        is_cloud_sure = self.is_cloud_sure(self, toa)

        return self._and(self._not(is_invalid), self._not(is_clear_snow), self._not(is_cloud_sure), tcl)

    @staticmethod
    def is_cloud(self, toa: tuple[DataTree, ...]) -> bool:
        return self._or(self.is_cloud_sure(self, toa), self.is_cloud_ambiguous(self, toa))

    @staticmethod
    def is_cirrus(self, toa: tuple[DataTree, ...]) -> bool:
        # TODO implement: we need elevation
        return False

    @staticmethod
    def is_cirrus_ambiguous(self, toa: tuple[DataTree, ...]) -> bool:
        # TODO implement: we need elevation
        return False

    @staticmethod
    def is_bright_white(self, toa: tuple[DataTree, ...]) -> bool:
        is_invalid = self.is_invalid(self, toa)
        bright_value = self.bright_value(self, toa)
        white_value = self.white_value(self, toa)

        return self._and(self._not(is_invalid), bright_value + white_value > BRIGHTWHITE_THRESH)

    @staticmethod
    def is_clear_snow(self, toa: tuple[DataTree, ...]) -> bool:
        # TODO implement: we need elevation
        return False

    @staticmethod
    def is_clear_land(self, toa: tuple[DataTree, ...]) -> bool:
        is_invalid = self.is_invalid(self, toa)
        if is_invalid:
            return False

        radiometric_land_value = self.radiometric_land_value(self, toa)
        a_priori__land_value = self.a_priori_land_value(self, toa)

        if self.almost_equals(radiometric_land_value, UNCERTAINTY_VALUE):
            land_value = radiometric_land_value
        elif a_priori__land_value > UNCERTAINTY_VALUE:
            land_value = a_priori__land_value
        else:
            return False

        is_cloud_sure = self.is_cloud_sure(self, toa)
        is_cloud_ambiguous = self.is_cloud_ambiguous(self, toa)
        is_cirrus = self.is_cirrus(self, toa)
        is_cirrus_ambiguous = self.is_cirrus_ambiguous(self, toa)

        return self._and(self._not(is_cloud_sure),
    ...
    @staticmethod
    def is_clear_water(self, toa: tuple[DataTree, ...]) -> bool:
        is_invalid = self.is_invalid(self, toa)
        if is_invalid:
            return False

        radiometric_water_value = self.radiometric_water_value(self, toa)
        a_priori_water_value = self.a_priori_water_value(self, toa)

        if self.almost_equals(radiometric_water_value, UNCERTAINTY_VALUE):
            water_value = radiometric_water_value
        elif a_priori_water_value > UNCERTAINTY_VALUE:
            water_value = a_priori_water_value
        else:
            return False
        ...
        is_cirrus_ambiguous = self.is_cirrus_ambiguous(self, toa)
        is_clear_snow = self.is_clear_snow(self, toa)
        is_bright_white = self.is_bright_white(self, toa)
    ...
    @staticmethod
    def is_invalid(self, toa: tuple[DataTree, ...]) -> bool:
        # TODO implement
        return False

    @staticmethod
    def is_b3_b11_water(self, toa: tuple[DataTree, ...]) -> bool:
        return self.b3_b11_value(self, toa) < ic.B3B11_THRESH

    @staticmethod
    def almost_equals(self, x1: float, x2: float) -> bool:
        return np.abs(x1 - x2) <= 1.E-6

    @staticmethod
    def tc4_value(self, toa: tuple[DataTree, ...]) -> DataTree:
        return -0.8239 * toa[1] + 0.0849 * toa[2] + 0.4396 * toa[3] - 0.058 * toa[8] + 0.2013 * toa[
            11] - 0.2773 * toa[12]

    @staticmethod
    def tc4_cirrus_value(self, toa: tuple[DataTree, ...]) -> DataTree:
        return self.tc4_value(self, toa) - toa[10]

    @staticmethod
    def tcl_value(self, toa: tuple[DataTree, ...]) -> DataTree:
        return 0.3029 * toa[1] + 0.2786 * toa[2] + 0.4733 * toa[3] + 0.5599 * toa[8] + 0.508 * toa[11] + 0.1872 * toa[
            12]

    @staticmethod
    def vis_bright_value(self, toa: tuple[DataTree, ...]) -> DataTree:
        return (toa[1] + toa[2] + toa[3]) / 3.0

    @staticmethod
    def b3_b11_value(self, toa: tuple[DataTree, ...]) -> DataTree:
        return toa[2] / toa[11]

    @staticmethod
    def ndwi_value(self, toa: tuple[DataTree, ...]) -> DataTree:
        return (toa[8] - toa[11]) / (toa[8] + toa[11])

    @staticmethod
    def ndsi_value(self, toa: tuple[DataTree, ...]) -> DataTree:
        return (toa[2] - toa[11]) / (toa[2] + toa[11])

    @staticmethod
    def ndvi_value(self, toa: tuple[DataTree, ...]) -> DataTree:
        ndvi = 0.5 * (1.0 + (toa[8] - toa[11]) / (toa[8] + toa[11]))
        return np.max(0.0, np.min(1.0, ndvi))

    @staticmethod
    def radiometric_land_value(self, toa: tuple[DataTree, ...]) -> float:
        if toa[7] >= toa[3]:
            return 1.0
        else:
    ...
    @staticmethod
    def radiometric_water_value(self, toa: tuple[DataTree, ...]) -> float:
        if toa[7] < toa[3]:
            return 1.0
        else:
            return UNCERTAINTY_VALUE
    ...
    @staticmethod
    def a_priori_land_value(self, toa: tuple[DataTree, ...]) -> float:
        return self.radiometric_land_value(toa)

    @staticmethod
    def a_priori_water_value(self, toa: tuple[DataTree, ...]) -> float:
        return self.radiometric_water_value(toa)

    @staticmethod
    def spectral_flatness_value(self, toa: tuple[DataTree, ...]) -> float:
        slope0 = self.spectral_slope(self, toa[1], toa[0], list(ic.wvls)[1], list(ic.wvls)[0])
        slope1 = self.spectral_slope(self, toa[2], toa[3], list(ic.wvls)[2], list(ic.wvls)[3])
        slope2 = self.spectral_slope(self, toa[4], toa[6], list(ic.wvls)[4], list(ic.wvls)[6])
    ...
    @staticmethod
    def bright_value(self, toa: tuple[DataTree, ...]) -> np.ndarray:
    ...
    @staticmethod
    def white_value(self, toa: tuple[DataTree, ...]) -> ndarray[tuple[Any, ...], dtype[Any]]:
        nodata_arr = np.zeros(shape=toa[0].shape)
        cond_nodata = self.bright_value(self, toa) <= BRIGHT_FOR_WHITE_THRESH
        spectral_flatness_value = self.spectral_flatness_value(self, toa)
        white_value = np.where(cond_nodata, nodata_arr, spectral_flatness_value)
        return white_value

    @staticmethod
    def spectral_slope(self, toa1: ndarray, toa2: ndarray, wvl1: float, wvl2: float) -> ndarray:
